...
- P0 实现采用 UTC 口径，而非本地时区。
- 今日 / 本周 / 本月均以 UTC 计算（今日：UTC 当天 00:00 起；本周：UTC 周一 00:00 起；本月：UTC 1 日 00:00 起；均为半开区间）。
- 理由：台账层既有 `period_start` / seed / recommend / ListExpiringPlans 等逻辑统一使用 UTC，避免混用本地时区造成区间错位。
- UI 仍展示「今日 / 本周 / 本月」，但底层统计口径为 UTC。
