...
type Handler struct {
	...
	planFormTmpl     *template.Template
...
func NewHandler(store *store.Store, tracker *tracker.Tracker, logger *slog.Logger, sessions *SessionStore, cfg *config.Config, zaiTracker ...*tracker.ZaiTracker) *Handler {
	...
	// Parse quota board page templates
	overviewTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_overview.html")
	if err != nil {
		logger.Error("failed to parse overview template", "error", err)
		overviewTmpl = template.New("empty")
	}
	plansTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_plans.html")
	if err != nil {
		logger.Error("failed to parse plans template", "error", err)
		plansTmpl = template.New("empty")
	}
	providersTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_providers.html")
	if err != nil {
		logger.Error("failed to parse providers template", "error", err)
		providersTmpl = template.New("empty")
	}
	usageTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_usage.html")
	if err != nil {
		logger.Error("failed to parse usage template", "error", err)
		usageTmpl = template.New("empty")
	}
	configTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_config.html")
	if err != nil {
		logger.Error("failed to parse config template", "error", err)
		configTmpl = template.New("empty")
	}
	risksTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_risks.html")
	if err != nil {
		logger.Error("failed to parse risks template", "error", err)
		risksTmpl = template.New("empty")
	}
	importExportTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_import_export.html")
	if err != nil {
		logger.Error("failed to parse import-export template", "error", err)
		importExportTmpl = template.New("empty")
	}

	// Parse quota board form templates
	platformFormTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_platform_form.html")
	if err != nil {
		logger.Error("failed to parse platform form template", "error", err)
		platformFormTmpl = template.New("empty")
	}
	planFormTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_plan_form.html")
	if err != nil {
		logger.Error("failed to parse plan form template", "error", err)
		planFormTmpl = template.New("empty")
	}
	bucketFormTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_bucket_form.html")
	if err != nil {
		logger.Error("failed to parse bucket form template", "error", err)
		bucketFormTmpl = template.New("empty")
	}
	riskFormTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_risk_form.html")
	if err != nil {
		logger.Error("failed to parse risk form template", "error", err)
		riskFormTmpl = template.New("empty")
	}

	modelFormTmpl, err := template.New("").ParseFS(templatesFS, "templates/layout.html", "templates/qb_model_form.html")
	if err != nil {
		logger.Error("failed to parse model form template", "error", err)
	...
	h := &Handler{
		...
		risksTmpl:        risksTmpl,
		importExportTmpl: importExportTmpl,

		platformFormTmpl: platformFormTmpl,
		planFormTmpl:     planFormTmpl,
