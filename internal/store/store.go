...
func (s *Store) createTables() error {
	schema := `
		...
		-- Quota board: platforms
		CREATE TABLE IF NOT EXISTS qb_platforms (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			name TEXT NOT NULL,
			...
			supports_tools_json INTEGER NOT NULL DEFAULT 0,
		...
		-- Quota board: plans
		CREATE TABLE IF NOT EXISTS qb_plans (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			platform_id INTEGER NOT NULL REFERENCES qb_platforms(id),
			name TEXT NOT NULL,
			...
			recommended_role TEXT NOT NULL DEFAULT '',
			risk_summary TEXT NOT NULL DEFAULT '',
			created_at TEXT NOT NULL,
			updated_at TEXT NOT NULL
		);

		-- Quota board: quota buckets
		CREATE TABLE IF NOT EXISTS qb_quota_buckets (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			plan_id INTEGER NOT NULL REFERENCES qb_plans(id),
		...
		-- Quota board: models
		CREATE TABLE IF NOT EXISTS qb_models (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			platform_id INTEGER NOT NULL REFERENCES qb_platforms(id),
			plan_id INTEGER REFERENCES qb_plans(id),
			model_id TEXT NOT NULL DEFAULT '',
			display_name TEXT NOT NULL DEFAULT '',
			family TEXT NOT NULL DEFAULT '',
			is_current INTEGER NOT NULL DEFAULT 0,
			base_url_override TEXT NOT NULL DEFAULT '',
			tool_fit_json TEXT NOT NULL DEFAULT '',
			status TEXT NOT NULL DEFAULT 'active',
			created_at TEXT NOT NULL,
			updated_at TEXT NOT NULL
		);

		-- Quota board: usage logs
		CREATE TABLE IF NOT EXISTS qb_usage_logs (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			plan_id INTEGER,
			model_id INTEGER,
		...
		-- Quota board: credential statuses
		CREATE TABLE IF NOT EXISTS qb_credential_statuses (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
			platform_id INTEGER NOT NULL REFERENCES qb_platforms(id),
		...
		-- Quota board: risk notes
		CREATE TABLE IF NOT EXISTS qb_risk_notes (
			id INTEGER PRIMARY KEY AUTOINCREMENT,
