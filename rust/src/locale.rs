...
locale_keys! {
    ...
    // FloatBarResetFormatNeedsInline: the floating bar prints no reset text
    // unless `float_bar_show_reset_inline` is on, so with it off the reset-time
    // mode reaches only the pill's hover tooltip. Shown on the FloatBar page
    // alone — the dashboard has no such gate.
    FloatBarResetFormatNeedsInline,
    // QuotaResetExpiredWaiting: the reset moment has passed but the provider
    // has not reported fresh numbers yet. Never render a negative countdown.
    QuotaResetExpiredWaiting,
    // QuotaResetUnknown: no `resets_at` and no provider-supplied description.
    // A reset time is never inferred.
    QuotaResetUnknown,
    ...
    // Settings V5 shell — nav ids, page copy, mock capability labels
    TabTrayPanel,
    TabTaskbarStatus,
    TabAppearance,
    TabPrivacy,
